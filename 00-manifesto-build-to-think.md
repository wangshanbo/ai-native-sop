...
> | OEM 工厂老板直达 | 6 类硬件 / 6 家工厂老板 —— **不是有手机号那么简单**：日常电话回访 + 硬件技术方案共同制定都在我这。例：主导灯控**通信协议演进 ZigBee → 433MHz → LoRa**、Android 点单机**主板换代**决策。**职位是"采购"，事实上是 6 个产品线的硬件架构负责人** |
> | 三方软硬件公司对接 | 6 家 |
...
## 在你开始读之前还有一件事 —— 我为什么写这些

哪怕大模型越来越强、工具层越来越薄，**最强最贵的模型也永远不会便宜到人人都用得起**。

所以我所有的方法论、副业、写作，都是为了那 **95%** 的开发者 —— 需要 ship 生产级代码、但**每个月不能拿一个工资来烧 token** 的人。

我相信的公式是：

> **70 分的模型 + 强 SOP，写出 80 分的代码 ≫ 80 分的模型 + 弱 prompt，写出 85 分但贵 10 倍的代码**

前者所有人能用。后者只服务付得起钱的 5%。**我做的所有东西，是为那 95% 做的。**

这篇 manifesto 是这套信仰的第一个切片，下面这套博客 10 篇是把它系统化。
